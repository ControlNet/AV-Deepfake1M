...
# Non-masked versions of the loss functions
class BMLoss(Module):
    """Non-masked version of MaskedBMLoss."""
...
class ContrastLoss(Module):
    ...
    def forward(self, pred1: Tensor, pred2: Tensor, labels: Tensor):
        ...
        for i in range(batch_size):
            # mean L2 distance squared
            d = torch.dist(pred1[i], pred2[i], 2)
            if labels[i]:
                # if is positive pair, minimize distance
                loss.append(d ** 2)
            else:
...
class BsnppLoss(Module):
    def __init__(self, cbg_feature_weight=0.01, prb_weight_forward=1):
        super().__init__()
        self.cbg_feature_weight = cbg_feature_weight
        self.prb_weight_forward = prb_weight_forward

        self.cbg_loss_func = MSELoss()
        self.cbg_feature_loss = BMLoss(MSELoss())
        self.bsnpp_pem_reg_loss_func = self.cbg_feature_loss

    def forward(self, pred_bm_p, pred_bm_c, pred_bm_p_c, pred_start, pred_end,
        pred_start_backward, pred_end_backward, gt_iou_map, gt_start, gt_end,
        feature_forward=None, feature_backward=None
    ):
        if self.cbg_feature_weight > 0:
            cbg_loss_forward = self.cbg_loss_func(pred_start, gt_start) + \
                self.cbg_loss_func(pred_end, gt_end)
            cbg_loss_backward = self.cbg_loss_func(torch.flip(pred_end_backward, dims=(1,)), gt_start) + \
                self.cbg_loss_func(torch.flip(pred_start_backward, dims=(1,)), gt_end)
        ...
        prb_reg_loss_p = self.bsnpp_pem_reg_loss_func(pred_bm_p, gt_iou_map)
        prb_reg_loss_c = self.bsnpp_pem_reg_loss_func(pred_bm_c, gt_iou_map)
        prb_reg_loss_p_c = self.bsnpp_pem_reg_loss_func(pred_bm_p_c, gt_iou_map)
        prb_loss = prb_reg_loss_p + prb_reg_loss_c + prb_reg_loss_p_c
